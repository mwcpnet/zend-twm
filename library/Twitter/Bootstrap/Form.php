...
abstract class Twitter_Bootstrap_Form extends Zend_Form {
     ...
     * Class constants
     */
    const DISPOSITION_FORM = '';
    const DISPOSITION_VERITICAL = 'vertical';
        ...
        if (!$this->_prefixesInitialized) {
            if (null !== $this->getView()) {
                $this->getView()->addHelperPath(
                        'Twitter/Bootstrap/View/Helper', 'Twitter_Bootstrap_View_Helper'
                );
            }

            $this->addPrefixPath(
                    'Twitter_Bootstrap_Form_Element', 'Twitter/Bootstrap/Form/Element', 'element'
            );

            $this->addElementPrefixPath(
                    'Twitter_Bootstrap_Form_Decorator', 'Twitter/Bootstrap/Form/Decorator', 'decorator'
            );

            $this->addDisplayGroupPrefixPath(
                    'Twitter_Bootstrap_Form_Decorator', 'Twitter/Bootstrap/Form/Decorator'
            );
    ...
    public function setDisposition($disposition) {
        if (
                in_array(
                        $disposition, array(
                    self::DISPOSITION_FORM,
                    self::DISPOSITION_HORIZONTAL,
                    self::DISPOSITION_VERITICAL,
                    self::DISPOSITION_INLINE,
                    self::DISPOSITION_SEARCH
                        )
                )
        ) {
            if ($disposition !== self::DISPOSITION_FORM) {
